...
define(['sitecore', 'dynatree'], function (_sc) {
  var control = {
    componentName: "ListManagementTreeView",
    selector: ".sc-listmanagementtreeview",
    control: "dynatree",
    namespace: "ui-",
    ...
    [
      ...
      { name: "filterTemplate", added: true}
    ],
    ...
    view:
    {
      ...
      reloadTree: function () {
        ...
        for (var n = 0; n < parts.length; n++) {
          var self = this,
              uri = parts[n].split(','),
              itemDislpayName = uri[0],
              databaseName = uri[1],
              itemId = uri[2],
              itemIcon = uri[3],
              databaseUri = new _sc.Definitions.Data.DatabaseUri(databaseName),
              itemUri = new _sc.Definitions.Data.ItemUri(databaseUri, itemId),
              database = new _sc.Definitions.Data.Database(databaseUri);
          database.getItem(itemId, function(item) {
            var rootNode = {
              rawItem: item,
              title: itemDislpayName,
              key: itemId,
              icon: self.model.get("showSitecoreIcons") ? itemIcon : "",
              itemUri: itemUri,
              path: item.$path,
              url: item.$url,
              select: true,
            ...
            if (self.widget) {
              self.widget.apply(self.$el, ["getRoot"]).removeChildren();
        ...
        database.getChildren(itemUri.getItemId(), function (items) {
          var res = [],
            filteredItems = _.filter(items, function (item) {
              var filterTemplate = self.model.get("filterTemplate");
              if (filterTemplate && (item.$templateId !== filterTemplate)) {
                return false;
              }
      ...
      appendLoadedChildren: function (parentNode, childrenNodes, destArray) {
        ...
        _.each(childrenNodes, function (item) {
          if (item.itemId != self.model.get("skipItemId")) {
            var newNode = {};
